                ...
                children: [
                  ...
                  if (r.status == 'in_transit') ...[
                    ...
                    if (isClient && r.driverPhone != null && r.driverPhone!.isNotEmpty)
                      _buildPhoneRow('Телефон водителя', r.driverPhone!),
                    if (isDriver && r.clientPhone != null && r.clientPhone!.isNotEmpty)
                      _buildPhoneRow('Телефон клиента', r.clientPhone!),
                  ],
                  const Divider(color: Colors.white24, height: 24),
        ...
        children: [
          ...
          InkWell(
            onTap: () {
              ...
              // В Flutter можно использовать url_launcher пакет
              // Для простоты показываем диалог с номером
              showDialog(
                context: context,
                builder: (context) => AlertDialog(
                  backgroundColor: const Color(0xFF2a2a2e),
                  title: const Text('Телефон', style: TextStyle(color: Colors.white)),
                  content: Text(phone, style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
                  actions: [
                    TextButton(
